...
class P4DummyDragTool:
	"""Drag tool for tuning the preset-4 dummy portrait positions in-game.

	Self-contained copy of the label drag tool logic, specialised for the dummy
	portraits: a left press on a registered portrait starts a drag that moves it
	with the mouse and records the delta vs its base position; the Snapshot
	button prints paste-ready "P4D <node>: (dx,dy)," lines for P4_DUMMY_MANUAL.
	...
	def run_ops(self):
		...
		return [
			...
			(str_store_string, s0, "@P4D d{reg4}: d=({reg2},{reg3})  x{reg0} y{reg1}"),
	...
	def snapshot_event_ops(self, header):
		...
		one paste-ready "P4D <node>: (dx,dy)," line per item, then confirms on
		the readout."""
		ops = [
		...
		for item in range(self.num_items):
			...
			ops.append((str_store_string, s0, "@P4D %d: ({reg0},{reg1})," % item))
...
# Slot base 20000 stays clear of the low slots used by other presentations and
# of the label tool's base 10000; slot numbers go up to 2^20 so this is safe.
p4_dummy_tool = P4DummyDragTool("p4_dummies", 20000, len(PRESET_4_UNITS))
