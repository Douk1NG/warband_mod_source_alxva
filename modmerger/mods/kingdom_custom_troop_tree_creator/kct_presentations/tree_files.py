...
def _build_load_ops():
	...
	for i in range(KTF_SLOT_COUNT):
		...
		ops.extend([
			...
			(try_end,),
			(str_store_string, s0, "@Slot {reg1}: {s2}"),
			(try_begin,),
...
def _build_event_ops():
	...
	ops.extend([
		...
		(else_try,),
			...
			(call_script, "script_kct_slot_is_occupied", "$kct_selected_slot"),
			(try_begin,),
				(eq, reg0, 0),
				(display_message, "@This slot is empty", 0xff0000),
			(else_try,),
				(call_script, "script_kct_slot_import_tree", "$kct_selected_slot"),
				(try_begin,),
					(eq, reg0, 1),
		...
		(else_try,),
			...
			(else_try,),
				...
				(display_message, "@This slot is empty", 0xff0000),
			(else_try,),
				(call_script, "script_kct_slot_clear", "$kct_selected_slot"),
				(start_presentation, "prsnt_kct_manage_tree_files"),
			(try_end,),
		(else_try,),
