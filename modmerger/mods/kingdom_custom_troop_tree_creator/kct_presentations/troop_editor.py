...
def _load_body():
	return [
		...
		(call_script, "script_kct_create_scrollable_container", KCT_INV_POS_X, KCT_INV_POS_Y, KCT_INV_AREA_W, KCT_INV_AREA_H),
		...
		(try_for_range, ":item_index", 0, ":capacity"),
			...
			(call_script, "script_kct_create_mesh_overlay", "mesh_inv_slot", ":pos_x", ":pos_y", KCT_INV_SLOT_FIXED, KCT_INV_SLOT_FIXED),
			(gt, ":item", 0),

			(troop_set_slot, "trp_cstm_overlay_items", reg1, ":item_slot"),

			(store_add, ":item_x", ":pos_x", KCT_INV_HALF_SLOT),
			(store_add, ":item_y", ":pos_y", KCT_INV_HALF_SLOT),
			(call_script, "script_kct_create_item_overlay", ":item", ":item_x", ":item_y", KCT_INV_SLOT_FIXED),
		(try_end),

		(set_container_overlay, -1),

		(str_store_string, s0, "@Right-click to remove"),
      (call_script, "script_kct_create_text_overlay", "str_s0", KCT_INV_POS_X, KCT_INV_BELOW_Y, 1000, KCT_INV_AREA_W, 20, tf_left_align),
		(assign, "$cstm_remove_from_inventory_message", reg1),
		(overlay_set_display, "$cstm_remove_from_inventory_message", 0),


		(call_script, "script_kct_create_combo_button_overlay", KCT_TYPE_COMBO_POS_X, KCT_TYPE_COMBO_POS_Y),
		(assign, "$cstm_store_item_type_selector", reg1),
		(position_set_x, pos1, 750),
		(position_set_y, pos1, 750),
		(overlay_set_size, "$cstm_store_item_type_selector", pos1),

		(call_script, "script_kct_create_combo_button_overlay", KCT_MOD_COMBO_POS_X, KCT_MOD_COMBO_POS_Y),
		...
		(call_script, "script_kct_create_combo_label_overlay", KCT_PAGE_LABEL_POS_X, KCT_PAGE_LABEL_POS_Y),
		...
		(store_add, ":num_pages", ":num_items", KCT_STORE_GRID_TOTAL - 1),
		(val_div, ":num_pages", KCT_STORE_GRID_TOTAL),
		...
		(call_script, "script_kct_create_scrollable_container", KCT_STORE_POS_X, KCT_STORE_POS_Y, KCT_STORE_AREA_W, KCT_STORE_AREA_H),
		(assign, "$cstm_store_container", reg1),

		(set_container_overlay, "$cstm_store_container"),

		(try_for_range, ":slot_no", 0, KCT_STORE_GRID_TOTAL),
			(store_mul, ":offset", "$cstm_item_page_no", KCT_STORE_GRID_TOTAL),
			...
			(call_script, "script_kct_get_grid_position", ":slot_no", KCT_STORE_GRID_TOTAL, KCT_STORE_CONT_WIDTH, KCT_STORE_SLOT_SIZE, KCT_STORE_SLOT_SIZE),
			(assign, ":pos_x", reg0),
			(assign, ":pos_y", reg1),


			(call_script, "script_kct_create_mesh_overlay", "mesh_inv_slot", ":pos_x", ":pos_y", KCT_STORE_SLOT_FIXED, KCT_STORE_SLOT_FIXED),
			(troop_set_slot, "trp_cstm_overlay_items", reg1, ":item"),
			(troop_set_slot, "trp_cstm_overlay_is_store_item", reg1, 1),

			(gt, ":item", 0),

			(store_add, ":item_x", ":pos_x", KCT_STORE_HALF_SLOT),
			(store_add, ":item_y", ":pos_y", KCT_STORE_HALF_SLOT),
			(call_script, "script_kct_create_item_overlay", ":item", ":item_x", ":item_y", KCT_STORE_SLOT_FIXED),
		(try_end),

		(set_container_overlay, -1),

		(str_store_string, s0, "@Remaining funds:"),
      (call_script, "script_kct_create_text_overlay", "str_s0", KCT_FUNDS_LABEL_POS_X, KCT_STORE_BELOW_Y, 1000, KCT_STORE_AREA_W, 20, tf_left_align),

		(call_script, "script_kct_troop_get_inventory_value", ":dummy"),
		(store_sub, ":remaining_funds", "$cstm_total_funds", reg0),
		(assign, reg0, ":remaining_funds"),
		(str_store_string, s0, "@{reg0} denars"),
      (call_script, "script_kct_create_text_overlay", "str_s0", KCT_FUNDS_VALUE_POS_X, KCT_STORE_BELOW_Y, 1000, 200, 20, tf_right_align),
		(try_begin),
		...
		(call_script, "script_kct_create_text_overlay", "str_s0", 0, KCT_LEVEL_HP_POS_Y, KCT_STATS_SKL_TEXT_SIZE, KCT_STATS_SIZE_X, 50, tf_left_align),
		...
		(assign, ":pos_x", reg0),
		(store_add, ":pos_y", reg1, KCT_ATTR_POS_Y),
		...
		(call_script, "script_kct_create_text_overlay", "str_s0", 0, KCT_ATTR_POINTS_POS_Y, KCT_STATS_POINTS_TEXT_SIZE, KCT_STATS_POINTS_COL_WIDTH, KCT_STATS_POINTS_ROW_HEIGHT, tf_left_align),
		...
		(call_script, "script_kct_create_text_overlay", "str_s0", 0, KCT_PROF_POINTS_POS_Y, KCT_STATS_POINTS_TEXT_SIZE, KCT_STATS_POINTS_COL_WIDTH * 2, KCT_STATS_PROF_POINTS_ROW_HEIGHT, tf_left_align),
      ...
      (call_script, "script_kct_create_text_overlay", "str_s0", KCT_NAME_COLUMN_X, KCT_NAME_POS_Y, 1000, KCT_NAME_LABEL_WIDTH, 50, tf_right_align),

      (str_store_troop_name, s0, ":dummy"),
      (call_script, "script_kct_create_text_box_overlay", "str_s0", KCT_NAME_COLUMN_X, KCT_NAME_POS_Y),
      (assign, "$cstm_set_name", reg1),

      (str_store_string, s0, "@Name (plural): "),
      (call_script, "script_kct_create_text_overlay", "str_s0", KCT_NAME_PLURAL_COLUMN_X, KCT_NAME_POS_Y, 1000, KCT_NAME_LABEL_WIDTH, 50, tf_right_align),

      (str_store_troop_name_plural, s0, ":dummy"),
      (call_script, "script_kct_create_text_box_overlay", "str_s0", KCT_NAME_PLURAL_COLUMN_X, KCT_NAME_POS_Y),
      (assign, "$cstm_set_name_plural", reg1),
		...
		(try_begin),
			...
			(call_script, "script_kct_create_game_button_overlay", "str_s0", KCT_BUTTON_RIGHT_X, KCT_BUTTONS_POS_Y),
...
def _post_load_additions():
	ops = []
	for item_type, string in cstm_item_type_strings.iteritems():
		ops.append((str_store_string, s0, "@" + string))
		ops.append((overlay_add_item, "$cstm_store_item_type_selector", s0))
	ops.extend([
	...
	ops.append((set_container_overlay, "$cstm_stats_container"))
	skill_index = 0
	for skill in ACTIVE_FIGHTING_SKILLS[::-1]:
		...
		ops.extend([
			...
			(assign, ":pos_x", reg0),
			(store_add, ":pos_y", reg1, KCT_SKL_GRID_Y),
	...
	ops.extend([
		...
		(call_script, "script_kct_create_text_overlay", "str_s0", 0, KCT_SKL_POINTS_POS_Y, KCT_STATS_POINTS_TEXT_SIZE, KCT_STATS_POINTS_COL_WIDTH, KCT_STATS_POINTS_ROW_HEIGHT, tf_left_align),
...
kct_customise_core = (
	...
	[
		(ti_on_presentation_load, _load_body() + _post_load_additions()),
		(ti_on_presentation_mouse_enter_leave, _mouse_enter_leave_body()),
		(ti_on_presentation_mouse_press, _mouse_press_body()),
		(ti_on_presentation_run, _run_body()),
		(ti_on_presentation_event_state_change, _event_state_change_body()),
	]
)
